...
app.get('/api/songs/all', authenticateToken, (req, res) => {
    const songs = readData(dataFiles.songs);
    const allSongs = [];
    for (const id in songs) {
        allSongs.push({
...
app.post('/api/songs', authenticateToken, upload.single('thumbnail'), (req, res) => {
    const { title, bpm, genre, thumbnail } = req.body;
    ...
    const newSong = {
        ...
        thumbnail: thumbnail || (req.file ? '/uploads/thumbnails/' + req.file.filename : generateRandomThumbnail(title)),
        createdAt: Date.now(),
        tracks: [],
        likes: 0,
        votes: [],
